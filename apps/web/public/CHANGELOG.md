...
- **Discord and GitHub OAuth callbacks no longer crash when Auth.js returns an immutable redirect response**: The NextAuth route wrapper now clones Auth.js responses before applying no-store and cookie headers, which fixes the production `TypeError: immutable` 500 during provider callback completion.
...
- **Settings now stay visually in sync with what was actually saved**: The settings mutation now returns the normalized saved preference record, and the settings page keeps an optimistic overlay until that saved value lands, so toggles and selects no longer snap back to older values after showing a success toast.
...
- **Windows installers are less likely to think Starchild is still running after you close it**: The Electron main process now tracks its bundled Next server child more defensively during shutdown, avoids duplicate async quit flows, and clears stale child-process references as soon as the server exits. This reduces cases where the NSIS installer reports that Starchild must be closed even though the window is already gone.
...
- **Windows Electron builds now try native rebuild automatically**: `electron:build:win` now starts with Electron Builder's native rebuild path enabled so Windows packaging no longer requires setting `STARCHILD_ELECTRON_WIN_NATIVE_REBUILD=true` just to attempt `@electron/rebuild`.

### Fixed

- **Windows packaging is still resilient on machines without C++ build tools**: If native rebuild fails because Visual Studio Build Tools or the `node-gyp` toolchain are unavailable, the Windows wrapper now retries once with rebuild disabled so packaging can still complete when recompiling native dependencies is unnecessary.
