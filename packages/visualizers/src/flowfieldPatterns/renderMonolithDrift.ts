...
export function renderMonolithDrift(
  ...
  const useColumnGradients = !p.isFirefox || detailScale >= 0.82;
  const useReflections = !p.isFirefox || detailScale >= 0.78;
  ...
  ctx.fillStyle = p.hsla(
    p.fastMod360(p.hueBase + 196),
    100,
    60,
    0.05 + bassIntensity * 0.08,
  );
  ctx.fillRect(
    0,
    horizonY - (6 + bassIntensity * 8),
    p.width,
    12 + bassIntensity * 16,
  );
  ...
  for (let index = 0; index < columnCount; index++) {
    ...
    if (useColumnGradients) {
      const fill = ctx.createLinearGradient(x, y, x, horizonY);
      fill.addColorStop(0, p.hsla(hue, 100, 82, 0.14 + audioIntensity * 0.16));
      fill.addColorStop(0.22, p.hsla(p.fastMod360(hue + 22), 98, 46, 0.18));
      fill.addColorStop(1, p.hsla(p.fastMod360(hue + 78), 88, 18, 0.03));
      ctx.fillStyle = fill;
    } else {
      ctx.fillStyle = p.hsla(
        hue,
    ...
    ctx.fillStyle = p.hsla(hue, 100, 88, 0.18 + trebleIntensity * 0.18);
    ...
    ctx.fillStyle = p.hsla(hue, 100, 88, 0.22 + trebleIntensity * 0.16);
    ctx.fillRect(x - width * 0.46, y - 2.5, width * 0.92, 2.5);

    if (useReflections || prominence > 0.52) {
      const reflection = ctx.createLinearGradient(
        x,
        horizonY,
        x,
        horizonY + height * 0.55,
      );
      reflection.addColorStop(
        0,
        p.hsla(hue, 100, 80, 0.1 + audioIntensity * 0.08),
      );
  ...
  const beamCount = Math.max(3, Math.min(5, (3 + detailScale * 2) | 0));
  for (let beam = 0; beam < beamCount; beam++) {
    const beamX = ((((time * 0.08 + beam * 0.19) % 1) + 1) % 1) * p.width;
    ...
    beamGradient.addColorStop(
      0.4,
      p.hsla(p.fastMod360(p.hueBase + 140 + beam * 28), 100, 82, 0.08),
    );
    beamGradient.addColorStop(
      1,
